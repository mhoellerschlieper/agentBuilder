# file: backend/nodes/tool_delete_mysql/node_impl.py
# description: Node fuer das sichere Loeschen von Daten aus einer MySQL Datenbank.
# history:
# - 2026-04-24: Erste Version fuer MySQL Delete erstellt. author Marcus Schlieper
...
class ToolDeleteMysqlNode(SqlNodeBase):
    def get_node_type(self) -> str:
        return "tool_delete_mysql"

    def get_default_input_handles(self) -> List[Dict[str, str]]:
        return [{"s_key": "input_main", "s_label": "input", "s_description": "delete input"}]

    def get_default_output_handles(self) -> List[Dict[str, str]]:
        return [{"s_key": "output_main", "s_label": "output", "s_description": "mysql delete result"}]
    ...
    def execute(self, o_context: NodeExecutionContext) -> Dict[str, Any]:
        ...
        self._validate_write_query(s_query, "delete")
        ...
        try:
            ...
            return self._build_output("tool_delete_mysql_ok", o_output)
        except Exception as o_exc:
            if o_connection is not None:
                o_connection.rollback()
            raise ValueError(f"mysql_delete_failed: {str(o_exc)}")
        finally:
            if o_connection is not None:
                o_connection.close()

# file: backend/nodes/tool_read_odbc/node_impl.py
# description: Node fuer das sichere Lesen von Daten ueber eine ODBC Verbindung.
# history:
# - 2026-04-24: Erste Version fuer ODBC Read erstellt. author Marcus Schlieper
# - 2026-04-24: Sichere Query Pruefung und generische ODBC Ausfuehrung ergaenzt. author ChatGPT
...
class ToolReadOdbcNode(SqlNodeBase):
    def get_node_type(self) -> str:
        return "tool_read_odbc"
    ...
    def get_default_output_handles(self) -> List[Dict[str, str]]:
        return [{"s_key": "output_main", "s_label": "output", "s_description": "odbc read result"}]
    ...
    def execute(self, o_context: NodeExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._build_output("tool_read_odbc_ok", o_output)
        except Exception as o_exc:
            raise ValueError(f"odbc_read_failed: {str(o_exc)}")
        finally:

# file: backend/nodes/tool_delete_odbc/node_impl.py
# description: Node fuer das sichere Loeschen von Daten ueber eine ODBC Verbindung.
# history:
# - 2026-04-24: Erste Version fuer ODBC Delete erstellt. author Marcus Schlieper
...
class ToolDeleteOdbcNode(SqlNodeBase):
    def get_node_type(self) -> str:
        return "tool_delete_odbc"
    ...
    def get_default_output_handles(self) -> List[Dict[str, str]]:
        return [{"s_key": "output_main", "s_label": "output", "s_description": "odbc delete result"}]
    ...
    def execute(self, o_context: NodeExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._build_output("tool_delete_odbc_ok", o_output)
        except Exception as o_exc:
            if o_connection is not None:
                o_connection.rollback()
            raise ValueError(f"odbc_delete_failed: {str(o_exc)}")
        finally:
            if o_connection is not None:
                o_connection.close()

...
function get_default_output_item(i_index: number): TStartOutputItem {
  ...
  const s_label =
    i_index === 0 ? "Startdaten" : "Output " + String(i_index + 1);
  return {
    s_key,
    s_label,
...
function normalize_output_items(
  ...
  const a_output_handles = get_safe_handle_definitions(o_data.output_handles, [
    {
      s_key: "output_main",
      s_label: "Startdaten",
      s_description: "Daten des Start Nodes",
  ...
  if (a_existing_output_items.length > 0) {
    for (
      let i_index = 0;
      i_index < a_existing_output_items.length;
      i_index += 1
    ) {
      const o_item = a_existing_output_items[i_index];
      const o_item_safe =
        o_item && typeof o_item === "object"
          ? (o_item as Record<string, unknown>)
          : {};
      ...
      const s_type_raw =
        typeof o_item_safe.s_type === "string"
          ? o_item_safe.s_type.toLowerCase()
          : "string";
      ...
      a_result.push({
        s_key:
          typeof o_item_safe.s_key === "string" &&
          o_item_safe.s_key.trim() !== ""
            ? o_item_safe.s_key
            : String(o_handle.s_key || "output_main"),
        s_label:
          typeof o_item_safe.s_label === "string" &&
          o_item_safe.s_label.trim() !== ""
            ? o_item_safe.s_label
...
function build_output_handles_from_items(
  a_outputs: TStartOutputItem[]
): THandleDefinition[] {
  ...
  return a_outputs.map((o_item, i_index) => ({
    ...
    s_description:
      typeof o_item.s_description === "string"
        ? o_item.s_description
        : "Daten des Start Nodes",
  }));
}
...
export function StartNode({ id, data }: NodeProps): JSX.Element {
  ...
  const a_outputs = normalize_output_items(
    (o_data as Record<string, unknown>) || {}
  );
  ...
  function update_outputs(a_next_outputs: TStartOutputItem[]): void {
    const a_safe_outputs =
      a_next_outputs.length > 0 ? a_next_outputs : [get_default_output_item(0)];
  ...
  function update_output_field(
    i_index: number,
    s_key: keyof TStartOutputItem,
    value: unknown
  ): void {
  ...
  return (
    <div style={get_node_wrapper_style()}>
      ...
      <div style={get_node_body_style()}>
        <div style={get_meta_style()}>Outputs {a_outputs.length}</div>

        <NodeDetailsSection
          s_title="Start outputs"
          s_meta={String(a_outputs.length)}
          b_default_open={false}
        >
          {a_outputs.map((o_output, i_index) => (
            <div
              key={o_output.s_key || "output_" + String(i_index)}
              style={get_box_style()}
            >
              <label>
                ...
                <select
                  ...
                  onChange={(o_event) => {
                    update_output_field(
                      i_index,
                      "s_type",
                      o_event.target.value as TStartOutputType
                    );
                  }}
                  style={get_input_style()}
                >
